...
import { INITIAL_MENU, INITIAL_RESTAURANT_INFO, INITIAL_CATEGORIES } from '../constants';
...
export const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  }, [categories, menuItems, restaurantInfo, orders]);
    ...
    if (supabase) {
      ...
      const fetchData = async () => {
        try {
          ...
          } else if (cats && cats.length === 0 && categories.length > 0) {
  ...
  }, [categories.length, menuItems.length, restaurantInfo]);
  ...
  const addCategory = async (name: string) => {
    ...
    if (supabase) {
      try {
        await supabase.from('categories').insert(newItem);
    ...
    // Fallback Local
    setCategories(prev => [...prev, { id: generateId(), ...newItem }]);
