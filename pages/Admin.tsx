...
const Admin = () => {
  ...
  const handleSaveItem = (e: React.FormEvent) => {
    ...
    if (itemToSave.name && itemToSave.categoryId && itemToSave.price !== undefined && !Number.isNaN(itemToSave.price)) {
      ...
      if (editingItem) {
        updateMenuItem(editingItem.id, { ...itemToSave, images });
        setEditingItem(null);
  ...
  const handleAddImage = (img: string) => setCurrentFormItem(prev => ({ ...prev, images: [...(prev.images || []), img] }));
  const handleRemoveImage = (index: number) => setCurrentFormItem(prev => ({ ...prev, images: prev.images?.filter((_, i) => i !== index) }));
    ...
    <div className="min-h-screen bg-gray-100 pb-20">
      ...
      <div className="p-4">
        ...
        {activeTab === 'menu' && (
          <>
            <button
              onClick={() => { setEditingItem(null); setNewItem(defaultNewItem); setIsAddingItem(!isAddingItem); }}
              ...
              {menuItems.map(item => (
                <div key={item.id} className="bg-white p-3 rounded-2xl shadow-sm flex items-center gap-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => { setEditingItem(item); setIsAddingItem(false); window.scrollTo({ top: 0, behavior: 'smooth' }) }} className="w-10 h-10 rounded-full bg-blue-50 text-blue-500 flex items-center justify-center">
                      <Edit3 size={20} />
                    </button>
                  </div>
                </div>
